...
const CONFIG = {
  ...
  majorVersion: '7.4',
  platform: 'win32-x64',
  downloadUrl: 'https://github.com/redis-windows/redis-windows/releases/download/7.4.1/redis-7.4.1-windows-x64.zip',
  checksum: 'sha256:PLACEHOLDER',
  binaries: [
    "src\\\\redis-server.exe",
    "src\\\\redis-cli.exe",
    "src\\\\redis-benchmark.exe",
    "src\\\\redis-check-aof.exe",
    "src\\\\redis-check-rdb.exe"
],
}

async function main() {
  const binDir = join(__dirname, 'bin')
  const serverBinary = join(binDir, 'src/redis-server.exe')
