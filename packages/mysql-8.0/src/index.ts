...
export const DATABASE_NAME = 'mysql'
export const DATABASE_DISPLAY_NAME = 'MySQL'
export const DATABASE_VERSION = '8.0.40'
export const MAJOR_VERSION = '8.0'

const binaries = {
  "server": "bin/mysqld",
  "client": "bin/mysql",
  "dump": "bin/mysqldump",
  "check": "bin/mysqlcheck",
  "installDb": "bin/mysqld --initialize-insecure",
  "printDefaults": "bin/my_print_defaults"
} as const

export type MySQLPaths = {
  basedir: string
  bindir: string
  server: string
  client: string
...
const PLATFORM_PACKAGE_MAP: Record<Platform, string> = {
  'linux-x64': '@host-db/mysql-8.0-linux-x64',
  'linux-arm64': '@host-db/mysql-8.0-linux-arm64',
  'darwin-arm64': '@host-db/mysql-8.0-darwin-arm64',
  'win32-x64': '@host-db/mysql-8.0-win32-x64',
}

export async function getBinaryPaths(): Promise<MySQLPaths> {
  ...
  try {
    ...
    throw new Error(
      `Platform package ${packageName} not found. ` +
        ...
        `Try reinstalling: npm install @host-db/mysql-8.0`,
    )
  }
